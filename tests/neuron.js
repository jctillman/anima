var neuronMaker = require('../src/neuron');
var neurons = require('../src/neurons');
var expect = require('chai').expect;

describe('Testing neuronMaker', function(){

	describe('Testing functions which every neuron in neuronMaker has', function(){
		...
		it('tests simple connection / diconnection items for all kinds', function(){
			...
			for(var x = 0 ; x < allNeuronKinds.length; x++){
				var m = neuronMaker({typeOfNeuron: allNeuronKinds[x] });
				var n = neuronMaker({typeOfNeuron: allNeuronKinds[x] });
				var o = neuronMaker({typeOfNeuron: allNeuronKinds[x] });
		...
		it('Tests that each neuron gives a positive value after being given a positive value', function(){
			...
			for(var x = 0 ; x < allNeuronKinds.length; x++){
				var m = neuronMaker({typeOfNeuron: allNeuronKinds[x], randomness: 'one' });
				var n = neuronMaker({typeOfNeuron: allNeuronKinds[x], randomness: 'one' });
				var o = neuronMaker({typeOfNeuron: allNeuronKinds[x], randomness: 'one' });
				o.connect(n);
				o.connect(m);
				m.init();
				n.init();
		...
		it('Calculates the derivative relative to some input correctly.', function(){
			...
			for(var x = 0; x < allNeuronKinds.length; x++){
				var m = neuronMaker({typeOfNeuron: 'input'});
				var n = neuronMaker({typeOfNeuron: 'input'});
				var o = neuronMaker({typeOfNeuron: allNeuronKinds[x], randomness: 'flatPositive', cost: 'squaredError' });
				var p = neuronMaker({typeOfNeuron: allNeuronKinds[x], randomness: 'flatPositive', cost: 'squaredError' });
		...
		it('Can handle the task of converting from one-spot to binary', function(){
			...
			for(var x = 0 ; x < allNeuronKinds.length; x++){
				...
				neurons = [[],[],[]];
				for(var a = 0; a < 8; a++){
					neurons[0].push(neuronMaker({typeOfNeuron: 'input'}));
				}
				for(var a = 0; a < 16; a++){
					neurons[1].push(neuronMaker({typeOfNeuron: allNeuronKinds[x], randomness: 'flatProportionateZero', cost: 'squaredError' }));
				}
				for(var a = 0; a < 3; a++){
					neurons[2].push(neuronMaker({typeOfNeuron: allNeuronKinds[x], randomness: 'flatProportionateZero', cost: 'squaredError' }));
				}
				...
				for(var a = 0; a < 8; a++){
					...
					for(z = 0; z < neurons.length; z++){
						h = ""
						for (y = 0; y < neurons[z].length; y++){
							h = h + " " + neurons[z][y].weights;
							if (neurons[z][y].weights.some(function(weight){
	...
	describe('Testing results specific to individual kinds of neurons', function(){

		it('Tests the not very leaky relu', function(){

			var one = function(num){return 1}

			var m = neuronMaker({typeOfNeuron: 'relu', randomness:'one'});
			var n = neuronMaker({typeOfNeuron: 'relu', randomness:'one'});
			var o = neuronMaker({typeOfNeuron: 'relu', randomness:'one'});
			...
			//Testing with input from two other relus
			m.activation(1);
			n.activation(1);
			o.activation();
			expect(o.a).to.equal(3);

			//Testing with input from one other relu
			o.disconnect(m);
			o.activation();
			expect(o.a).to.equal(2);

			//Testing with input from a relu set to zero -- just with bias.
			n.activation(0)
			o.activation();
			expect(o.a).to.equal(1);

			//Hook up again, this time ready for propogation.
			o.connect(m);
			m.activation(1);
			n.activation(1);
			o.activation();
			...
			expect(m.dCwrtA).to.equal(3)
			expect(n.dCwrtA).to.equal(3)
